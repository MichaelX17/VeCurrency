...
export const useRates = () => {
  ...
  const fetchBinancePrice = async (): Promise<number> => {
    try {
      const response = await fetchWithTimeout('https://p2p.binance.com/bapi/c2c/v2/friendly/c2c/adv/search', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ page: 1, rows: 1, payTypes: [], asset: 'USDT', tradeType: 'SELL', fiat: 'VES', transAmount: '' })
      });
      ...
      if (e instanceof Error && e.name === 'AbortError') throw new Error('BINANCE_TIMEOUT');
